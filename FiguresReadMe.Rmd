...
N_{i}(t+1) = \frac{s_{i}\lambda_iN_i(t)}{1+\Sigma^n_{j=1}\alpha_{ij}\lambda_{i}N_{j}(t)}
...
M_1 = init_metacom(size, nspp, 10, each = T)
M_2 = init_metacom(size, nspp, 10)
...
The following chunks parses and plot data of population dynamics (Figure 2 and Figure S4 for Scenarios 1 and 2, respectively). For analyses using other intraspecific competition coefficients (Figure S8-S13), change the `intra` values below accordingly. After simulations, make sure to reset `intra` to the default value (0.07) before running other simulations.
...
num_lowdisp = multiple_reps(M_1, season_length, n_seasons, disp_rates, 50, disp_freq, disp_freq_seasons,
                            lambda, surv, intra, B, scal, xmid, 0, end_season_surv, max_time, var, rep = 50, n_disturb = nd, burnin = 10)
dynamics_num_lowdisp = met_df_multirep(num_lowdisp[[1]]) %>% filter(!(time == 0)) %>% mutate(time = time) %>% mutate(type = "num")

tra_lowdisp = multiple_reps(M_1, season_length, n_seasons, disp_rates, 50, disp_freq, disp_freq_seasons,
                            lambda, surv, intra, B, scal, xmid, 1, end_season_surv, max_time, var, rep = 50, n_disturb = nd, burnin = 10)
dynamics_tra_lowdisp = met_df_multirep(tra_lowdisp[[1]]) %>% filter(!(time == 0)) %>% mutate(time = time) %>% mutate(type = "tra")

lowdisp = rbind(dynamics_num_lowdisp, dynamics_tra_lowdisp) %>% mutate(disp_rate = 0.01)

# High dispersal, Scenario 1

disp_rates = rep(0.1, nspp)

num_highdisp = multiple_reps(M_1, season_length, n_seasons, disp_rates, 50, disp_freq, disp_freq_seasons,
                             lambda, surv, intra, B, scal, xmid, 0, end_season_surv, max_time, var, rep = 50, n_disturb = nd, burnin = 10)
dynamics_num_highdisp = met_df_multirep(num_highdisp[[1]]) %>% filter(!(time == 0)) %>% mutate(time = time) %>% mutate(type = "num")

tra_highdisp = multiple_reps(M_1, season_length, n_seasons, disp_rates, 50, disp_freq, disp_freq_seasons,
                             lambda, surv, intra, B, scal, xmid, 1, end_season_surv, max_time, var, rep = 50, n_disturb = nd, burnin = 10)
dynamics_tra_highdisp = met_df_multirep(tra_highdisp[[1]]) %>% filter(!(time == 0)) %>% mutate(time = time) %>% mutate(type = "tra")

highdisp = rbind(dynamics_num_highdisp, dynamics_tra_highdisp) %>% mutate(disp_rate = 0.1)
...
rbind(lowdisp, highdisp) %>% filter(patch == 5) %>% 
...
The following chunk plots final occupancy of the dominant species in each repetition, defined as the proportion of patches in which the regionally dominant species has the highest population; the regional dominant species is determined as the species that has the highest regional population for each repetition. This chunk produces Figure 3.

This chunk uses data generated by the previous section.

```{r}

all_pop_dynamics = rbind(lowdisp %>% filter(time == 110) %>% mutate(init_cond = "single_spp"), 
                         highdisp %>% filter(time == 110) %>% mutate(init_cond = "single_spp"), 
                         lowdisp_2 %>% filter(time == 110) %>% mutate(init_cond = "equal_pop"), 
                         highdisp_2 %>% filter(time == 110) %>% mutate(init_cond = "equal_pop"))
...
max_pop = all_pop_dynamics %>% group_by(patch, rep, type, disp_rate, init_cond) %>% filter(population == max(population))
...
all_counts %>% mutate(init_cond = fct_relevel(init_cond, "single_spp", "equal_pop")) %>% 
  ggplot(aes(x = as.factor(disp_rate), y = occupancy, fill = type)) + geom_boxplot() + 
  stat_summary(fun = median, geom = "point", size = 3, shape = 21, aes(fill = type), color = "black",
               position = position_dodge(0.75), show.legend = F) +
  theme_bw() + xlab("Dispersal rate") + ylab("Occupancy of the\ndominant species") + ylim(0, 1) +
  scale_x_discrete(label = c("Low", "High")) +
  scale_color_manual(values = c("#5A72D1", "#D97437")) +
  scale_fill_manual(values = c("#5A72D1", "#D97437"), labels = c("No trait-mediated", "Trait-mediated")) + 
  facet_grid(. ~ init_cond, scales = "free_y", 
             labeller = labeller(init_cond = c(single_spp = "1. Numeric advantage", equal_pop = "2. Equal initial"))) + 
  theme(axis.text.y = element_text(size = 15), axis.text.x = element_text(size = 15), axis.ticks = element_blank(), 
        axis.title.y = element_text(size = 20), axis.title.x = element_text(size = 20), 
...
all_counts %>% mutate(init_cond = fct_relevel(init_cond, "single_spp", "equal_pop")) %>% 
  group_by(type, disp_rate, init_cond) %>% summarize(mean = mean(occupancy), sd = sd(occupancy)) %>% 
  ggplot(aes(x = as.factor(disp_rate), y = mean, color = type)) + 
  geom_errorbar(aes(ymin = mean-sd, ymax = mean+sd), width = 0.3, position = position_dodge(0.75)) + 
...
## Population at Differnt Times

The following chunks calculate population of each species of one randomly selected patch at different times of the simulation (Figure S2, S5, for Scenarios 1 and 2, respectively). 

This chunk uses data generated by the [Population Dynamics] section.

```{r}

# Figure S3 (Scenario 1)
...
The following chunks produce figures of dispersal-diversity relationships under two initial scenarios (Figure 4). For analyses using other intraspecific competition coefficients (Figure S10, S13), change the `intra` values below accordingly. After simulations, make sure to reset `intra` to the default value (0.07) before running other simulations.
...
num_Sce1 = multiple_disp(M_1, season_length, n_seasons, disp_rates_lst, c(50), disp_freq, disp_freq_seasons,
                         lambda, intra, B, scal, xmid, 0, max_time, var = var, rep = 50, burnin = 10)
tra_Sce1 = multiple_disp(M_1, season_length, n_seasons, disp_rates_lst, c(50), disp_freq, disp_freq_seasons,
                         lambda, intra, B, scal, xmid, 1, max_time, var = var, rep = 50, burnin = 10)
...
num_Sce2 = multiple_disp(M_2, season_length, n_seasons, disp_rates_lst, c(50), disp_freq, disp_freq_seasons,
                         lambda, intra, B, scal, xmid, 0, max_time, var = var, rep = 50)
tra_Sce2 = multiple_disp(M_2, season_length, n_seasons, disp_rates_lst, c(50), disp_freq, disp_freq_seasons,
                         lambda, intra, B, scal, xmid, 1, max_time, var = var, rep = 50)
...
rbind(mutate(all_Sce1, init_cond = "single_spp"), 
      mutate(all_Sce2, init_cond = "equal_pop")) %>% mutate(init_cond = fct_relevel(init_cond, "single_spp", "equal_pop")) %>% 
  ...
  facet_grid(metric ~ init_cond, scales = "free_y", 
             labeller = labeller(metric = c(alpha = "Alpha", beta = "Beta", gamma = "Gamma"),
                                 init_cond = c(single_spp = "1. Numeric advantage", equal_pop = "2. Equal initial"))) + 
  theme(axis.text.y = element_text(size = 15), axis.text.x = element_text(size = 15), axis.ticks = element_blank(), 
        axis.title.y = element_text(size = 20), axis.title.x = element_text(size = 20), 
        legend.text = element_text(size = 17), legend.title = element_blank(), 
        panel.grid.minor = element_blank(), 
...
The following chunks generate figures that show how dispersal-diversity relationships are affected by the magnitude of seasonal variations (Figure S3, S6 for Scenarios 1 and 2, respectively).
...
var = 0.5
num_var1 = multiple_disp(M_1, season_length, n_seasons, disp_rates_lst, c(50), disp_freq, disp_freq_seasons,
                         lambda, intra, B, scal, xmid, 0, max_time, var = var, rep = 50, burnin = 10)
tra_var1 = multiple_disp(M_1, season_length, n_seasons, disp_rates_lst, c(50), disp_freq, disp_freq_seasons,
                         lambda, intra, B, scal, xmid, 1, max_time, var = var, rep = 50, burnin = 10)

num_var1 %<>% mutate(type = "num")
tra_var1 %<>% mutate(type = "tra")
all_var1 = rbind(num_var1, tra_var1)# %>% pivot_longer(cols = 2:4, names_to = "metric", values_to = "values")

# Medium var

var = 2

num_var2 = multiple_disp(M_1, season_length, n_seasons, disp_rates_lst, c(50), disp_freq, disp_freq_seasons,
                         lambda, intra, B, scal, xmid, 0, max_time, var = var, rep = 50, burnin = 10)
tra_var2 = multiple_disp(M_1, season_length, n_seasons, disp_rates_lst, c(50), disp_freq, disp_freq_seasons,
                         lambda, intra, B, scal, xmid, 1, max_time, var = var, rep = 50, burnin = 10)

num_var2 %<>% mutate(type = "num")
tra_var2 %<>% mutate(type = "tra")
all_var2 = rbind(num_var2, tra_var2)# %>% pivot_longer(cols = 2:4, names_to = "metric", values_to = "values")

# High var

var = 8

num_var3 = multiple_disp(M_1, season_length, n_seasons, disp_rates_lst, c(50), disp_freq, disp_freq_seasons,
                         lambda, intra, B, scal, xmid, 0, max_time, var = var, rep = 50, burnin = 10)
tra_var3 = multiple_disp(M_1, season_length, n_seasons, disp_rates_lst, c(50), disp_freq, disp_freq_seasons,
                         lambda, intra, B, scal, xmid, 1, max_time, var = var, rep = 50, burnin = 10)

num_var3 %<>% mutate(type = "num")
tra_var3 %<>% mutate(type = "tra")
all_var3 = rbind(num_var3, tra_var3)# %>% pivot_longer(cols = 2:4, names_to = "metric", values_to = "values")
...
var = 0.5
num_var12 = multiple_disp(M_2, season_length, n_seasons, disp_rates_lst, c(50), disp_freq, disp_freq_seasons,
                         lambda, intra, B, scal, xmid, 0, max_time, var = var, rep = 50)
tra_var12 = multiple_disp(M_2, season_length, n_seasons, disp_rates_lst, c(50), disp_freq, disp_freq_seasons,
                         lambda, intra, B, scal, xmid, 1, max_time, var = var, rep = 50)

num_var12 %<>% mutate(type = "num")
tra_var12 %<>% mutate(type = "tra")
all_var12 = rbind(num_var12, tra_var12)# %>% pivot_longer(cols = 2:4, names_to = "metric", values_to = "values")

# Medium var

var = 2
num_var22 = multiple_disp(M_2, season_length, n_seasons, disp_rates_lst, c(50), disp_freq, disp_freq_seasons,
                         lambda, intra, B, scal, xmid, 0, max_time, var = var, rep = 50)
tra_var22 = multiple_disp(M_2, season_length, n_seasons, disp_rates_lst, c(50), disp_freq, disp_freq_seasons,
                         lambda, intra, B, scal, xmid, 1, max_time, var = var, rep = 50)

num_var22 %<>% mutate(type = "num")
tra_var22 %<>% mutate(type = "tra")
all_var22 = rbind(num_var22, tra_var22)# %>% pivot_longer(cols = 2:4, names_to = "metric", values_to = "values")

# High var

var = 8
num_var32 = multiple_disp(M_2, season_length, n_seasons, disp_rates_lst, c(50), disp_freq, disp_freq_seasons,
                         lambda, intra, B, scal, xmid, 0, max_time, var = var, rep = 50)
tra_var32 = multiple_disp(M_2, season_length, n_seasons, disp_rates_lst, c(50), disp_freq, disp_freq_seasons,
                         lambda, intra, B, scal, xmid, 1, max_time, var = var, rep = 50)

num_var32 %<>% mutate(type = "num")
tra_var32 %<>% mutate(type = "tra")
all_var32 = rbind(num_var32, tra_var32)# %>% pivot_longer(cols = 2:4, names_to = "metric", values_to = "values")
...
rbind(mutate(all_var1, var = as.factor(0.5)), 
      mutate(all_var2, var = as.factor(2)), 
      mutate(all_var3, var = as.factor(8))) %>%
  pivot_longer(cols = 2:4, names_to = "metric", values_to = "value") %>% 
  ggplot(aes(x = disp_rate, y = value, color = type, fill = type)) + geom_smooth() +
  theme_bw() + xlab("Dispersal rate") + ylab("Value") + 
  scale_x_continuous(breaks = c(0, 0.05, 0.1), labels = c("0", "0.05", "0.1")) +
...
rbind(mutate(all_var12, var = as.factor(0.5)), 
      mutate(all_var22, var = as.factor(2)), 
      mutate(all_var32, var = as.factor(8))) %>%
  pivot_longer(cols = 2:4, names_to = "metric", values_to = "value") %>% 
  ggplot(aes(x = disp_rate, y = value, color = type, fill = type)) + geom_smooth() +
  theme_bw() + xlab("Dispersal rate") + ylab("Value") + 
  scale_x_continuous(breaks = c(0, 0.05, 0.1), labels = c("0", "0.05", "0.1")) +
...
tb_low_num1 = temporal_beta(num_lowdisp[[1]], step = 10) %>% filter(time > 20) %>% mutate(type = "num", disp_rate = 0.01)
tb_low_tra1 = temporal_beta(tra_lowdisp[[1]], step = 10) %>% filter(time > 20) %>% mutate(type = "tra", disp_rate = 0.01)
tb_high_num1 = temporal_beta(num_highdisp[[1]], step = 10) %>% filter(time > 20) %>% mutate(type = "num", disp_rate = 0.1)
tb_high_tra1 = temporal_beta(tra_highdisp[[1]], step = 10) %>% filter(time > 20) %>% mutate(type = "tra", disp_rate = 0.1)
